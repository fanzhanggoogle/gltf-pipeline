    ...
    ForEach.animation(gltf, function(animation) {
      ForEach.animationSampler(animation, function(sampler) {
        if (defined(sampler.extensions) &&
            defined(sampler.extensions.Draco_animation_compression)) {
          if (sampler.extensions.Draco_animation_compression.bufferView > bufferViewId)
            sampler.extensions.Draco_animation_compression.bufferView--;
        }
      });
    });
};
