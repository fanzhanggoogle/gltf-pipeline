  ...
  /*
    ...
    for (var accessorId in accessors) {
        if (accessors.hasOwnProperty(accessorId)) {
            ...
            if (accessor.hasOwnProperty('bufferView'))
...
function packBuffers(gltf) {
  ...
  // Count the size of the final buffer.
  var totalBufferSize = 0;
  var allBufferData = [];
  for (var bufferViewId in bufferViews) {
    var bufferView = bufferViews[bufferViewId];
    console.log('BufferView id ' + bufferViewId + ', offset ' + bufferView.byteOffset);
    var buffer = gltf.buffers[bufferView.buffer];
    console.log('Get data from buffer ' + bufferView.buffer);
    console.log('Data size ' + bufferView.byteLength + ' / ' + buffer.extras._pipeline.source.byteLength);
    var bufferData = buffer.extras._pipeline.source.slice(bufferView.byteOffset, bufferView.byteLength);
    if (bufferData.byteLength === 0) {
      console.log('Error: Get data from buffer failed.');
    }
    console.log('Data size is ' + bufferData.byteLength);
    allBufferData.push(bufferData);
    bufferView.byteOffset = totalBufferSize;
    // Only one buffer will left.
    bufferView.buffer = 0;
    totalBufferSize += bufferView.byteLength;

    console.log('Total size now ' + totalBufferSize);
  }
  var packedBuffer = Buffer.concat(allBufferData);
    
  console.log('Total packed buffer size ' + packedBuffer.byteLength);
  var buffers = [{
