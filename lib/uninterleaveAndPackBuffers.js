...
function uninterleaveAndPackBuffers(gltf) {
    ...
    // Mark if bufferView is belong to accessor.
    var accessorForBufferViews = {};
    ...
    ForEach.accessor(gltf, function (accessor) {
        ...
        accessorForBufferViews[accessor.bufferView] = true;
    ...
    if (defined(gltf.extensionsUsed) &&
      ...
      var bufferViewIdOffset = 3;
      var compressionBufferViews = [];
      ForEach.mesh(gltf, function(mesh) {
        ForEach.meshPrimitive(mesh, function(primitive, primitiveId) {
          if (defined(primitive.extensions) &&
              defined(primitive.extensions.KHR_draco_mesh_compression)) {
            var extension = primitive.extensions.KHR_draco_mesh_compression;
            var bufferView = gltf.bufferViews[extension.bufferView];
            extension.bufferView += bufferViewIdOffset;
