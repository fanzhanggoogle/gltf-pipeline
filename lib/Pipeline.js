...
var compressAnimations = require('./compressAnimations.js');
...
Pipeline.processJSONWithExtras  = function(gltfWithExtras, options) {
    ...
    */
    ...
    // The following are commented because they are currently not well supported.
/*
...
*/
    if (options.compressAnimations) {
      compressAnimations(gltfWithExtras, options.dracoAnimationOptions);
    }

    // TODO: Add quantization.
    if (options.compressMeshes) {
      // Add default byteOffset.
      ForEach.bufferView(gltfWithExtras, function(bufferView) {
        if (!defined(bufferView.byteOffset))
          bufferView.byteOffset = 0;
      });
      ForEach.accessor(gltfWithExtras, function(accessor) {
        if (!defined(accessor.byteOffset))
          accessor.byteOffset = 0;
      });
      compressMeshes(gltfWithExtras, options.dracoOptions);
    }
/*
